...
module.exports = app => {
  ...
  app.put("/api/todos", (req, res) => {
    if (req.user) {
      if (req.body.data.completed) {
        db.Todo.updateOne({ _id: req.body.data.id }, { completed: true })
          .then(() => {
            res.send("todo completed");
          })
          .catch(err => console.log(err));
      } else if (req.body.data.finished) {
        db.Todo.updateOne({ _id: req.body.data.id }, { finished: true })
          .then(() => {
  ...
  app.post("/api/todos", (req, res) => {
    if (req.user) {
      db.Todo.create({ todo: req.body.data.todo }).then(todoDoc => {
        db.User.updateOne(
          { _id: req.user._id },
